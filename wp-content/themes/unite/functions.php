...
add_action( 'init', 'film_Country_init' );

function film_genres_init() {
    // create a new taxonomy
    register_taxonomy(
        'film-genres',
        'films',
        array(
            'label' => __( 'Genres' ),
            'rewrite' => array( 'slug' => 'genres-category' ),
            'hierarchical' => true,
        )
    );
}
add_action( 'init', 'film_genres_init' );

function film_Year_init() {
    // create a new taxonomy
    register_taxonomy(
        'film-year',
        'films',
...
add_action( 'init', 'film_Year_init' );

function film_actors_init() {
    // create a new taxonomy
    register_taxonomy(
        'film-actors',
        'films',
        array(
            'label' => __( 'Actors' ),
            'rewrite' => array( 'slug' => 'actors-category' ),
            'hierarchical' => true,
        )
    );
}
add_action( 'init', 'film_actors_init' );

function latest_five_movies(){
    $html_structure = '<div class="custom_films"><ul class="inner_custom_films">';
    $args = array(
                    'post_type' => 'films',
                    'post_status' => 'publish',
                    'orderby' => 'date',
    ...
    if ( $the_query->have_posts() ) {
        while ( $the_query->have_posts() ) {
            $the_query->the_post();
            $html_structure .= '<li style="display: inline-block;"><h3><a href="'.get_permalink().'" title="'.get_the_title().'">'.get_the_title().'</a></h3></li>';
        } // end while
    } 
    return $html_structure .= '</ul></div>';
}
add_shortcode('latest_five','latest_five_movies');
